...
# LOAD DATA
data_set = dataset_cyclegan.his_dataset()
...
dice_loss = loss1.diceloss().cuda()
dice_ssim_loss = loss.dicessim(0.5, 0.5).cuda()
...
for epoch in range(params.num_epochs):
    for step, (real_A, real_B) in enumerate(train_loader):
        ...
        # cycle_A_loss = l1ssim()(recon_A, real_A)
        # cycle_A_loss = 1 - pytorch_ssim.ssim(recon_A, real_A)
        # cycle_A_loss = MSE_loss(recon_A, real_A)
        ...
        if epoch == params.num_epochs - 1:
            if (step + 1) % 20 == 0:
                ...
                save_name = os.path.join(logpathnew,
                                         'generator_A_param_' + 'Epoch' + str(epoch + 1) + '_step' + str(step + 1) + '.pkl')
                torch.save(G_A.state_dict(),
                           save_name)

                save_name2 = os.path.join(logpathnew,
                                          'generator_B_param_' + 'Epoch' + str(epoch + 1) + '_step' + str(
                                              step + 1) + '.pkl')
